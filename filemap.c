				    ...
				    int color_on)
{
	...
	if (ra->mmap_miss > 0)
		ra->mmap_miss--;
	if (PageReadahead(page)) {
		if(color_on) color_page_cache_async_readahead(mapping, ra, file,
					   	page, offset, ra->ra_pages);
		else page_cache_async_readahead(mapping, ra, file,
					   page, offset, ra->ra_pages);
	}
}
#else
...
again:
		...
		/* --ADDED-- */
		int color_on = 0;
		if(check_apps != NULL && check_apps(file)) {
